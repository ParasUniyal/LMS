...
useEffect(()=>{
  ...
  return (
    <>
    <div id='main_panel' className=' h-lvh ml-6  bg-white border border-slate-300 '>
         <input className='rounded-full mt-6 ml-4 leading-8 pl-4 border border-slate-400' placeholder='Search Module...'
                type='text'
                value={input} 
         ...
         <div>
              <div className='flex flex-wrap'>
                 {filter.map((map)=>{
                    return(
                        <div className='bg-slate-100 w-96 h-28 rounded-lg ml-4 mt-4 border-b-4 border-sky-800'>
                        <div className='text-sky-600 pl-4 flex place-content-between'>
                            <div>{map.head}</div>
                            <i onClick={show} class="fa-solid fa-pen"></i>
                            {modal && <Modaldata show={show}/>}
                        </div>
                    <div className='flex'>
                        <div className='bg-white w-24 h-14 mt-2.5 ml-4 border-b-4 border-green-700  rounded-md'>
